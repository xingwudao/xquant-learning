...
2. 阅读以下 oxq 模块的源码，了解 `RunResult` 的评估接口：
   ...
   - `result.calmar_ratio()` — Calmar 比率（年化收益 / 最大回撤）
   - `result.sortino_ratio()` — Sortino 比率（年化收益 / 下行波动率）
...
7. 构造买入持有基准——等权买入 3 只 ETF，之后不做任何交易：
   ...
   - 计算基准的累计收益、年化收益、年化波动率、最大回撤、夏普比率

8. 打印策略 vs 基准对比表：
   - 列：策略名、累计收益、年化收益、波动率、最大回撤、夏普、Calmar、Sortino
   - 最后一行是等权买入持有基准
   - 额外打印超额收益（策略累计 - 基准累计）
...
10. 打印分析（根据实际数据动态描述方向，不要硬写"更高"或"更低"）：
    - 哪些策略跑赢了基准？哪些没有？
    - 超额收益最大的是哪个？但 Calmar 和 Sortino 的排名呢？
    - 「年化 15% 好不好？取决于同期等权买入持有赚了多少。跑赢基准的部分，才是你的'本事'——多出来的叫超额收益。」
    - 「夏普、Calmar、Sortino 是三把不同的尺子。夏普看总波动，Calmar 看最大回撤，Sortino 只看下行波动。你最怕什么风险，就用什么尺子。」
