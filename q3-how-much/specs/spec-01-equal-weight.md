...
5. 使用 `YFinanceDownloader` 下载 3 只 ETF 数据，数据窗口固定为 `2021-01-01` 到 `2026-03-18`。
   不要使用 `today`、`now()` 或默认截止到最新数据的下载方式。
   `yfinance` 的 `end` 参数是排他边界；代码中可用 `2026-03-19` 作为下载结束参数，再按 `END = "2026-03-18"` 读取。
   下载调用用 `refresh_yfinance(symbol, START, DOWNLOAD_END)` 包装；如果下载失败，打印错误并继续使用本地缓存。
...
使用 `START = "2021-01-01"`、`END = "2026-03-18"` 和当前 SDK 复现时，参考结果为：
- 沪深300ETF：累计收益率 `-2.11%`，年化波动率 `18.06%`，最大回撤 `-42.16%`
- 纳指100ETF：累计收益率 `106.44%`，年化波动率 `22.64%`，最大回撤 `-27.50%`
- 黄金ETF：累计收益率 `171.81%`，年化波动率 `15.83%`，最大回撤 `-16.77%`
