...
- 已定义的变量：`result_base`、`equity`、`daily_ret`、`monitor`、`SYMBOLS`、`NAMES`、`START`、`END`、`DATA_END`、`CN_MARKET_CALENDAR`、`market`、`make_strategy`、`make_rules`、`FEE_MODEL`、`BEST_FREQ`、`BEST_SL`
...
2. 诊断 1——执行落差排查：
   ...
   - "模拟实盘"的 `SimBroker` 必须传入 `market_calendar=CN_MARKET_CALENDAR`
   - 两次回测都使用 `start=START`、`end=DATA_END`
...
4. 诊断 3——参数稳定性检查：
   ...
   - 两半段实验中的 `FillPriceMode.NEXT_OPEN` broker 都必须传入 `market_calendar=CN_MARKET_CALENDAR`
   - 后半段结束日期使用 `DATA_END`
