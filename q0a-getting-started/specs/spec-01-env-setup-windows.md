...
学员刚在 TRAE 中打开了一个空目录作为课程学习根目录。本机是 Windows 10 或 11，尚未安装 Python 或任何依赖。这是课程的第一份 spec——后续 35 份 spec 都依赖这一份装出的环境。

## 任务

在当前目录下完成 Windows 量化课程编程环境配置：装 uv → 用 uv 创建 Python 3.12 虚拟环境 → 装 7 个依赖包 → 创建并跑通 `check_env.py` 自检脚本。

## 要求

### 第 0 步：用 PowerShell 而非 cmd

本 spec 所有命令在 **PowerShell** 中运行，不是 cmd。
打开方法：开始菜单搜索 `PowerShell`，点击「Windows PowerShell」或「PowerShell 7」。
如果不确定打开的是哪个，运行 `$PSVersionTable.PSVersion` 应输出版本号。
...
（PowerShell 的 `pwd` 等价于 mac 的 `pwd`，输出当前目录路径。）记录这个路径——后续所有操作都在这个目录下进行。不要 `cd` 切换到别的目录。
...
**装完后关闭并重新打开 PowerShell**——Windows 上 PATH 在当前会话不会自动生效，必须重开窗口。然后验证：

```powershell
uv --version
```

应输出 uv 的版本号。如果提示 `uv 不是可识别的命令`，说明终端没重开，关闭再开一次。
...
PowerShell 的换行续命用反引号 `` ` ``，与 mac/Linux 的 `\` 不同。
...
| 现象 | 处理 |
|---|---|
| `uv 不是可识别的命令` | 关闭并重新打开 PowerShell（让 PATH 生效） |
| pip install 慢 / 超时 | 已用清华镜像；仍超时把报错完整复制给 AI |
| 用户名是中文（如 `C:\Users\张三\`） | uv 或 pip 可能在路径处理上报编码错误。在 D 盘等英文路径下创建课程目录（如 `D:\quant-learning\`），把 TRAE 重新指向那里 |
| 任何其他报错 | 把终端完整输出（含命令 + 报错）复制给 AI 助手排查，**不要自己改命令重试** |
| 每次打开新 PowerShell | 先运行 `.venv\Scripts\activate` 激活虚拟环境 |
